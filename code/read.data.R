# Load the expert-provided population labels for the 1000 Genomes
# Phase 3 samples from the text file that was downloaded from the 1000
# Genomes FTP site.
read.1kg.labels <- function (filename) {
  out <- read.table(filename,header = TRUE,as.is = FALSE)
  names(out)[1] <- "id"
  return(out[c("id","pop")])
}

# Read an n x p genotype matrix from a .traw file, where n is the
# number of samples and p is the number of genetic markers (SNPs). See
# http://www.cog-genomics.org/plink2/formats#traw for more information
# about this file format.
#
# This function will be slow to read the file for a large genotype
# data file, so use cautiously.
#
read.geno.traw <- function (fam.file, geno.file) {
  ids     <- read.table(fam.file,stringsAsFactors = FALSE)[[1]]
  geno    <- read.table(geno.file,sep = " ",header = TRUE,as.is = "SNP")
  markers <- geno$SNP
  geno    <- t(as.matrix(geno[-(1:6)]))
  rownames(geno) <- ids
  colnames(geno) <- markers
  mode(geno)     <- "double"
  return(geno)
}

# Read the text file storing the PC matrix---that is, the mapping of
# the genotype samples onto PCs---generated by the MATLAB script
# geno_pca.m.
read.pcs <- function (filename)
  read.table(filename,header = TRUE,stringsAsFactors = FALSE)

# Read a matrix from a space-delimited text file created by MATLAB
# script write_rot_matrix.m.
read.rot.matrix <- function (filename) {
  out <- read.table(filename,sep = " ",header = TRUE,stringsAsFactors = FALSE)
  rownames(out) <- out$marker
  return(as.matrix(out[-1]))
}

# Read the mean genotypes from a space-delimited text file created by
# write_mean_genotypes.m.
read.mean.genotypes <- function (filename) {
  out      <- read.table(filename,sep = " ",header = TRUE,
                         stringsAsFactors = FALSE)
  y        <- out$mean
  names(y) <- out$marker
  return(y)
}

# Read the estimated admixture proportions from an ADMIXTURE output
# file (with a .Q suffix).
read.admix <- function (fam.file, admix.file) {
  ids <- read.table(fam.file,stringsAsFactors = FALSE)[[1]]
  out <- read.table(admix.file)
  n   <- ncol(out)
  out <- cbind(ids,out)
  colnames(out) <- c("id",paste("K",1:n,sep=""))
  return(out)
}
